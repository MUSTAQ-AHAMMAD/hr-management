...
<aside x-data="{ open: false }" class="w-64 bg-navy-900 text-white flex-shrink-0 hidden md:flex md:flex-col">
    ...
    <nav class="flex-1 overflow-y-auto py-4">
        <div class="px-3 space-y-1">
            ...
            <!-- Tasks -->
            @can('view tasks')
            <a href="{{ route('tasks.index') }}" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors duration-150 {{ request()->routeIs('tasks.*') ? 'bg-navy-800 text-white' : 'text-gray-300 hover:bg-navy-800 hover:text-white' }}">
                <svg class="h-5 w-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
